  ...
  private List<VFSFile> getRoot( VFSFile fileDomainRoot ) throws FileException {
    ...
    try {
      vfsConnectionDetails = (VFSConnectionDetails) bowl.getConnectionManager()
        .getConnectionDetails( getConnectionName( fileDomainRoot ) );
    ...
    for ( VFSRoot root : vfsRoots ) {
      ...
      vfsDirectory.setParent( fileDomainRoot.getPath() );
      vfsDirectory.setPath( fileDomainRoot.getPath() + VFSFile.DELIMITER + root.getName() );
